...
typedef struct {
	unsigned char e_ident[EI_NIDENT]; /* Magic number and other info */ // 目标文件标识
	...
	Elf32_Word e_flags;		  /* Processor-specific flags */  // 相关处理器的标志
...
typedef struct {
	...
	Elf32_Word sh_flags;	 /* Section flags */  //对应Section在进程中执行的特性（读、写）
	Elf32_Addr sh_addr;	 /* Section addr */   //对应Section在内存中的虚拟地址
	Elf32_Off sh_offset;	 /* Section offset */ //对应Section在文件中的偏移
	Elf32_Word sh_size;	 /* Section size */   //对应Section的大小
	Elf32_Word sh_link;	 /* Section link */
	Elf32_Word sh_info;	 /* Section extra info */
	Elf32_Word sh_addralign; /* Section alignment */
	Elf32_Word sh_entsize;	 /* Section entry size */
} Elf32_Shdr; //sh是section header的意思
