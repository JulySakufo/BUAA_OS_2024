...
void __attribute__((noreturn)) sys_yield(void) {
	...
	schedule(1); //set the yield，yield!=0,切换进程
}
...
int sys_mem_alloc(u_int envid, u_int va, u_int perm) {
	...
	try(envid2env(envid, &env, 1)); //perm用hint给的，一直保持非0
...
int sys_mem_unmap(u_int envid, u_int va) {
	...
	try(envid2env(envid, &e, 1)); //考虑到其他情况会返回错误的代码
...
void do_syscall(struct Trapframe *tf) {
	...
	arg4 = *(u_int *)(tf->regs[29] + 16); //取出内存中的值，因此要先转换到指针指向内存，再解引用
	arg5 = *(u_int *)(tf->regs[29] + 20);
	/* Step 5: Invoke 'func' with retrieved arguments and store its return value to $v0 in 'tf'.
	 */
	/* Exercise 4.2: Your code here. (4/4) */
	tf->regs[2] = func(arg1, arg2, arg3, arg4, arg5); //寄存器的位置可以参考mars 2号对应v0 29对应sp
}
