...
static struct Env_list env_free_list; // Free list  它的field是env_link，通过link实现结构体指针之间的连接
...
static void map_segment(Pde *pgdir, u_int asid, u_long pa, u_long va, u_int size, u_int perm) {
	...
	for (int i = 0; i < size; i += PAGE_SIZE) {
		...
		struct Page *pp = pa2page(pa+i); //得到对应的物理页
		page_insert(pgdir, asid, pp, va+i, perm);//将va映射到对应的物理页面
	}
}
 ...
 */
u_int mkenvid(struct Env *e) { //为进程申请env_id
	static u_int i = 0;
 ...
 */
static int env_setup_vm(struct Env *e) { //初始化新进程的地址空间
	/* Step 1:
	...
	p->pp_ref++;
	e->env_pgdir = (Pde *)page2kva(p); //页目录地址
	...
	memcpy(e->env_pgdir + PDX(UTOP), base_pgdir + PDX(UTOP),
	       sizeof(Pde) * (PDX(UVPT) - PDX(UTOP))); //把这部分内存对应的内核页表 base_pgdir 拷贝到进程页表中,暴露是 UTOP 往上到 UVPT 之间所有进程共享的只读空间
...
int env_alloc(struct Env **new, u_int parent_id) {
	...
	r = env_setup_vm(e); //看env_setup_vm是否成功了
	if(r!=0){
		return r;
	}
	...
	e->env_id = mkenvid(e);
	r = asid_alloc(&(e->env_asid)); //asid 是有限的,当所有的 asid 都被分配以后，应该返回异常。
	if(r!=0){
		return r;
	}
 ...
 *   segments.
 */
static void load_icode(struct Env *e, const void *binary, size_t size) { //加载可执行文件 binary 到进程 e 的内存中
	 ...
	 */
	size_t ph_off;
	ELF_FOREACH_PHDR_OFF (ph_off, ehdr) { //解析出每个 segment 的段头 ph，以及其数据在内存中的起始位置 bin，再由 elf_load_seg 函数将参数指定的程序段（program segment）加载到进程起始位置 bin，再由 elf_load_seg 函数将参数指定的程序段（program segment）加载到进程
	...
	e->env_tf.cp0_epc = ehdr->e_entry; //set A to B意思是把B的值赋给A
}
