...
void *disk_addr(u_int blockno) { //将数据块编号转换为缓冲区范围内的对应虚拟地址
...
void *block_is_mapped(u_int blockno) { //获取对应磁盘块的虚拟起始地址
...
int map_block(u_int blockno) { //将数据块编号转换为缓冲区范围内的对应虚拟地址
	...
	try(syscall_mem_alloc(env->env_id, disk_addr(blockno), PTE_D)); //系统调用分配页进行map操作(函数里面已经有page_insert了)
}
...
void free_block(u_int blockno) {
	...
	bitmap[blockno / 32] |= 1 << (blockno % 32); //1代表空闲
}
...
int copy_file_content(struct File *src, struct File *dst) {
   ...
   for (u_int i = 0; i < nblock; i++) {
      ...
      memcpy(dst_blk, src_blk, BLOCK_SIZE); //finish the copy?
      try(file_dirty(dst, i * BLOCK_SIZE)); //is it right?*/
...
int copy_directory_contents(struct File *src, struct File *dst) {
   ...
   if ((r = file_get_block(src, i, &blk)) < 0) {
	 ...
	 if(dst_file->f_type == FTYPE_REG){
		 //try(copy_file_content(dir_content, dst_file)); //unsure
		 copy_file_content(&dir_content[j], dst_file);
	 }else if(dst_file->f_type == FTYPE_DIR){
		 //try(copy_directory_contents(dir_content, dst_file)); //unsure
		 copy_directory_contents(&dir_content[j], dst_file);
	 }
	 //try(file_dirty(dst, j * BLOCK_SIZE)); //unsure
