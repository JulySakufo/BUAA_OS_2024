 ...
 */
void ide_read(u_int diskno, u_int secno, void *dst, u_int nsecs) { //实现磁盘与软件的操作，软件提供需要，磁盘通过MALTA_IDE_DATA发回来即可，存在dst这个磁盘缓存地址
	uint8_t temp;
	...
	while (secno < max) {
		...
		temp = 1;
		panic_on(syscall_write_dev(&temp, MALTA_IDE_NSECT, 1)); //write，前写后；read，后写前。write和read是相当于va的行为而言的
