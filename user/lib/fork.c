...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	...
	syscall_mem_alloc(0, (void *)UCOW, perm); //处于user mode，不能使用kern中的page_alloc，而要采用系统调用，这样UCOW(虚拟地址)映射到一个物理页面
	/* Step 4: Copy the content of the faulting page at 'va' to 'UCOW'. */
	/* Hint: 'va' may not be aligned to a page! */
	/* Exercise 4.13: Your code here. (4/6) */
	memcpy((void *)UCOW, (void *)ROUNDDOWN(va, PAGE_SIZE), PAGE_SIZE); //按照页大小对齐,将va对应的物理页面内容拷贝进UCOW(物理页面内容的拷贝)
	// Step 5: Map the page at 'UCOW' to 'va' with the new 'perm'.
	/* Exercise 4.13: Your code here. (5/6) */
	syscall_mem_map(0, (void *)UCOW, 0, (void *)va, perm); //参考sys_mem_map的作用，使得va映射到UCOW映射的页面
	// Step 6: Unmap the page at 'UCOW'.
	/* Exercise 4.13: Your code here. (6/6) */
	syscall_mem_unmap(0, (void *)UCOW); //参考sys_mem_unmap的作用，解除UCOW和它之前映射的页面的关系，这样就实现了只有va映射到新的页面，修改也是在新的页面中修改
	// Step 7: Return to the faulting routine.
	int r = syscall_set_trapframe(0, tf); //换言之，UCOW是一个中间变量，用来建立va与新的物理页面的桥梁，原来的映射关系没有变，新的关系也建立了
...
static void duppage(u_int envid, u_int vpn) {
	...
	syscall_mem_map(0, (void *)addr, envid, (void *)addr, perm); //子进程的修改
	if(r!=0){	
		syscall_mem_map(0, (void *)addr, 0, (void *)addr, perm);  //父进程的修改，传入的envid为0是为了对应envid2env取自己的方便操作
	}
}
...
int fork(void) {
	...
	if (child == 0) {
		env = envs + ENVX(syscall_getenvid()); //此时的env是子进程
		return 0; //子进程返回0
	}
	...
	for(i=0;i<VPN(USTACKTOP);i++){ //遍历所有below USTACKTOP的页
		if((vpd[i>>10] & PTE_V) && (vpt[i] & PTE_V)){ //判断page是否被map了
			duppage(child, i); //要将地址空间中需要与子进程共享的页面映射给子进程
		}
	}
