...
static int pipe_read(struct Fd *fd, void *vbuf, u_int n, u_int offset) {
	...
	p = fd2data(fd);
	rbuf = (char *)vbuf; //读者读到的数据
	for(i=0; i<n; i++){
		while(p->p_rpos >= p->p_wpos){ //如果管道数据为空即当 p_rpos >= p_wpos 时，应该进程切换到写者运行。

			if(i > 0 || _pipe_is_closed(fd, p)){ //读到缓冲区的尽头了或者缓冲区关闭了，返回读到的数目
				return i;
			}else{ //一点数据都没有读过并且管道还没被关，等待写者写
				syscall_yield();
			}
		}
		rbuf[i] = p->p_buf[p->p_rpos % PIPE_SIZE]; //移动读者读到的指针
		p->p_rpos++; //移动缓冲区内容的指针
	}
	return n; //成功读完n个字符
...
static int pipe_write(struct Fd *fd, const void *vbuf, u_int n, u_int offset) {
	...
	wbuf = (char *)vbuf;
	for(i=0; i<n; i++){
		while(p->p_wpos - p->p_rpos >= PIPE_SIZE){ //pipe buffer满了
...
static int pipe_close(struct Fd *fd) {
	...
	syscall_mem_unmap(0, fd); //解除进程对于fd的映射
	syscall_mem_unmap(0, (void *)fd2data(fd)); //解除进程对于pipe的映射
	return 0;
}
